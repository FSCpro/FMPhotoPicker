    ...
    private func updateInfoBar() {
        ...
        if let selectedIndex = self.dataSource.selectedIndexOfPhoto(atIndex: self.currentPhotoIndex) {
            if self.config.selectMode == .multiple {
                ...
                self.selectedIcon.image = UIImage(named: "check_on", in: .module, compatibleWith: nil)
            } else {
                self.selectedIndex.isHidden = true
                self.selectedIcon.image = UIImage(named: "single_check_on", in: .module, compatibleWith: nil)
            }
        } else {
            self.selectedIndex.isHidden = true
            self.selectedIcon.image = UIImage(named: "check_off", in: .module, compatibleWith: nil)
        }
...
//        private weak var unsafeAreaBottomView: UIView!
        ...
        backButton.setImage(UIImage(named: "icon_back", in: .module, compatibleWith: nil), for: .normal)
